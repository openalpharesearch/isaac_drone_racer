...
class RCBuffer:
    def __init__(self, timeout_sec: float = 0.25):
        """
        timeout_sec: if no new RC msg arrives within this window, we auto-clear to neutral.
        """
    ...
    def get(self, consume: bool = True):
        ...
        out = self._rc.copy()
        if consume:
            # one-shot behavior: clear after use
            self._has_cmd = False
            self._rc[:] = 0.0
        return out, True
...
def main():
    ...
    while simulation_app.is_running():
        ...
        if has_cmd:
            # Use RC override immediately
            actions_np = rc_to_actions(rc_vec)  # [FR, RL, FL, RR]
        else:
            # No action unless RC override is taken: hold neutral zeros
        ...
        # --- Step env (still step so publisher runs at sim rate) ---
